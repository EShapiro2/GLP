...
void main() {
  group('PartialEvaluator guard validation', () {
    ...
    test('mixed guards: builtin and single-unit-clause both accepted', () {
      const source = '''
        procedure my_unify(_, _).
        my_unify(X, X?).

        procedure test(_, _).
        test(X?, Y) :- integer(X?), my_unify(Y, X?) | process(Y?).
    ...
    test('unit clause from prelude (new_channel) accepted in guard', () {
      // This test verifies that prelude unit clauses work
      // Note: Requires prelude to be loaded, which may need different setup
      // For now, we simulate with our own new_channel
      const source = '''
