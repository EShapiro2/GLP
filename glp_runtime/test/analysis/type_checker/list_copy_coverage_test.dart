...
//
// Tests for mode coverage checking with Bimodal type
...
void main() {
  group('Mode coverage for Bimodal type', () {

    test('Bimodal INPUT arg - single writer clause, incomplete coverage FAILS', () {
      // Bimodal ::= _ ; _? requires BOTH modes covered
      // Declared INPUT (Bimodal?) → callee sees OUTPUT → expects WRITER
      final typeDecl = '''
Bimodal ::= _ ; _?.
procedure test(Bimodal?).
''';
      ...
      expect(hasCoverageError, isTrue,
        reason: 'Bimodal requires both modes, only writer provided');
    });

    test('Bimodal INPUT arg - two clauses covering both modes PASSES', () {
      // Declared INPUT (Bimodal?) → callee sees OUTPUT
...
''';
    ...
    test('Custom bimodal type requires coverage', () {
      // Universal ::= _ ; _? same semantics as Bimodal
      final typeDecl = '''
Universal ::= _ ; _?.
...
''';
      ...
      expect(hasCoverageError, isTrue,
        reason: 'Custom bimodal type should require coverage');
    });
