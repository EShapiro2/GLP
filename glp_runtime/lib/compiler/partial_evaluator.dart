...
class PartialEvaluator {
  ...
  /// Built-in defined guards (virtual unit clauses).
  /// These are treated as if defined by unit clauses in the program.
  /// =(X, X) - unification as defined guard
  Map<String, List<Term>> _getBuiltinUnitClauses() {
    // =(X, X) - both args are the same variable (writer)
    // This enables compile-time unification: A = B becomes aliasing B to A
    final xVar = VarTerm('X', false, 0, 0); // writer X
    return {
      '=/2': [xVar, xVar], // =(X, X)
    };
  }
  ...
  /// Built-in unit clauses (like =/2) are included automatically.
  Map<String, List<Term>> _collectUnitClauses(Program program) {
    final Map<String, List<Term>> unitClauses = {};

    // Add built-in unit clauses first
    unitClauses.addAll(_getBuiltinUnitClauses());

    // Then user-defined (can override built-ins if needed)
    for (final proc in program.procedures) {
  ...
  Clause _transformClause(Clause clause, Map<String, List<Term>> unitClauses) {
    ...
    // Add ground guards for readers that appear multiple times (SRSW compliance)
    final neededGroundGuards = _findReadersNeedingGroundGuards(currentHead, currentGuards, currentBody);
    for (final varName in neededGroundGuards) {
      // Check if ground guard already exists
      bool hasGroundGuard = currentGuards.any((g) =>
        g.predicate == 'ground' &&
        g.args.length == 1 &&
        g.args[0] is VarTerm &&
        (g.args[0] as VarTerm).name == varName
      );
      if (!hasGroundGuard) {
        currentGuards.add(Guard(
          'ground',
          [VarTerm(varName, true, clause.line, clause.column)],
          clause.line,
          clause.column,
        ));
      }
  ...
  /// Find reader variables that need ground guards due to multiple occurrences
  Set<String> _findReadersNeedingGroundGuards(Atom head, List<Guard> guards, List<Goal>? body) {
    final readerCounts = <String, int>{};

    // Count readers in head
    _countReaders(head.args, readerCounts);

    // Count readers in guards
    for (final guard in guards) {
      _countReaders(guard.args, readerCounts);
    }

    // Count readers in body
    if (body != null) {
      for (final goal in body) {
        _countReaders(goal.args, readerCounts);
      }
    }

    // Return readers that appear more than once
    return readerCounts.entries
      .where((e) => e.value > 1)
      .map((e) => e.key)
      .toSet();
  }

  /// Count reader variable occurrences in a list of terms
  void _countReaders(List<Term> terms, Map<String, int> counts) {
    for (final term in terms) {
      _countReadersInTerm(term, counts);
    }
  }

  /// Count reader variable occurrences in a term
  void _countReadersInTerm(Term term, Map<String, int> counts) {
    if (term is VarTerm && term.isReader) {
      counts[term.name] = (counts[term.name] ?? 0) + 1;
    } else if (term is StructTerm) {
      _countReaders(term.args, counts);
    } else if (term is ListTerm) {
      if (term.head != null) _countReadersInTerm(term.head!, counts);
      if (term.tail != null) _countReadersInTerm(term.tail!, counts);
    }
  }
  ...
  UnifyResult? _unifyTerms(
    ...
    // Check if unitArg is a variable already bound in subst
    // This handles cases like =(X, X) where X appears twice
    if (unitArg is VarTerm && subst.containsKey(unitArg.name)) {
      final existing = subst[unitArg.name]!;
      // callArg must equal the existing binding
      // If callArg is a variable, bind it to existing
      if (callArg is VarTerm && !callArg.isReader) {
        subst[callArg.name] = existing;
        return null;
      } else if (callArg is VarTerm && callArg.isReader) {
        // Reader - need to check compatibility
        suspSet.add(callArg.name);
        return null;
      } else {
        // callArg is a constant/structure - check compatibility with existing
        return _unifyTerms(existing, callArg, subst, suspSet);
      }
    }
  ...
  /// For clause heads, tracks seen writers to convert duplicates to readers (SRSW compliance)
  Atom _applySubstitutionToAtom(Atom atom, Map<String, Term> subst) {
    final seenWriters = <String>{};
    final newArgs = atom.args.map((a) => _applySubstitutionSRSW(a, subst, seenWriters)).toList();
    return Atom(
      atom.functor,
      newArgs,
      atom.line,
  ...
  /// Apply substitution with SRSW tracking for heads
  /// When a writer variable is seen twice, convert the second to a reader
  Term _applySubstitutionSRSW(Term term, Map<String, Term> subst, Set<String> seenWriters) {
    if (term is VarTerm) {
      if (term.name == '_') return term; // underscore unchanged
      final varName = term.name;
      if (subst.containsKey(varName)) {
        final replacement = subst[varName]!;
        // Recursively apply SRSW to the replacement
        return _applySubstitutionSRSW(replacement, subst, seenWriters);
      }
      // Variable not in substitution - check SRSW
      if (!term.isReader) {
        // It's a writer - check if we've seen it
        if (seenWriters.contains(varName)) {
          // Second occurrence - convert to reader
          return VarTerm(varName, true, term.line, term.column);
        }
        seenWriters.add(varName);
      }
      return term;
    }
    if (term is StructTerm) {
      return StructTerm(
        term.functor,
        term.args.map((a) => _applySubstitutionSRSW(a, subst, seenWriters)).toList(),
        term.line,
        term.column,
      );
    }
    if (term is ListTerm) {
      if (term.isNil) return term;
      return ListTerm(
        term.head != null ? _applySubstitutionSRSW(term.head!, subst, seenWriters) : null,
        term.tail != null ? _applySubstitutionSRSW(term.tail!, subst, seenWriters) : null,
        term.line,
        term.column,
      );
    }
