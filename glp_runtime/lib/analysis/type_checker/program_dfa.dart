...
/// instantiation and adds it to the environment.  A type built from a
/// constructed call-site argument (sec:param-procedures: "the concrete type at
/// the call site ... of an argument that is a constructed term [is] the type of
/// the term itself") is such a definition, and it is discovered while a clause
/// is being checked rather than at the declaration-driven expansion, so it is
/// added here instead of being built into the DFA from the start.
///
/// The addition is purely additive --- the two states `T` and `T?` and their
/// two automata, under a name nothing else uses --- so every state and
/// automaton already in [dfa] is left as it was, and a second call for the same
/// name is a no-op.  [types] must already carry [typeDef] under its own name,
/// so a self-referential alternative and [primitiveClosure] resolve.
