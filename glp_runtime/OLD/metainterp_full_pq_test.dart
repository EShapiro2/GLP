...
void main() {
  test('Full metainterpreter: run(q(X?)), run(p(X)) with object program p(a), p(b), q(b)', () {
    print('\n' + '=' * 70);
    print('FULL METAINTERPRETER TEST');
    print('Object program: p(a). p(b). q(b).');
    print('Encoded as: clause(p(a),true). clause(p(b),true). clause(q(b),true).');
    print('');
    ...
    print('Expected: X binds to a, then q(a) fails');
    print('=' * 70 + '\n');
    ...
    // This allows cross-predicate calls within the same program
    ...
    final prog = BC.prog([
      ...
      // Clause 3: clause(q(b), true).
      BC.L('clause/2_clause3'),
      BC.TRY(),
      BC.headStruct('q', 1, 0),        // arg 0 = q(_) structure
      BC.unifyConst('b'),               // First arg of q(...) is 'b'
      BC.headConst('true', 1),          // arg 1 = 'true'
      BC.COMMIT(),
      BC.PROCEED(),
