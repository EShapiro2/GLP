...
class TypeChecker {
  ...
  TypeCheckResult check(List<Clause> clauses) {
    final errors = <TypeError>[];
    final warnings = <TypeWarning>[];
    
    // Group clauses by procedure (name/arity)
    final procedureClauses = <String, List<Clause>>{};
    for (final clause in clauses) {
      final key = '${clause.headFunctor}/${clause.headArity}';
  ...
  TypeCheckResult _checkProcedure(ProcDecl decl, List<Clause> clauses) {
    final errors = <TypeError>[];
    final warnings = <TypeWarning>[];
  ...
  ClauseCheckResult _checkClause(Clause clause, ProcDecl decl, List<TypeDFA> argDFAs) {
    final errors = <TypeError>[];
    final warnings = <TypeWarning>[];
    
    // Step 1: Check head arguments match declared types
    if (clause.headArgs.length != decl.arity) {
      errors.add(TypeError(
        'Clause head arity ${clause.headArgs.length} does not match declaration ${decl.arity}',
        clause.line, clause.column, clause.text
      ));
    ...
    for (int i = 0; i < clause.headArgs.length; i++) {
      final arg = clause.headArgs[i];
    ...
    // Step 3: Check body goals use variables consistently with inferred types
    for (final goal in clause.bodyGoals) {
  ...
  GroundPathCheck _checkGroundPaths(Term term, TypeDFA dfa) {
    final paths = term.groundPaths;
  ...
  GoalCheckResult _checkGoal(Goal goal, Map<String, TypeDFA> varTypes) {
    final errors = <TypeError>[];
...
// =============================================================================
// Placeholder AST types for GLP terms
// These should match the actual GLP compiler AST
// =============================================================================

/// A GLP clause
class Clause {
  final String headFunctor;
  final List<Term> headArgs;
  final List<Goal> bodyGoals;
  final int line;
  final int column;
  final String text;
  
  Clause({
    required this.headFunctor,
    required this.headArgs,
  ...
  int get headArity => headArgs.length;
}

/// A GLP term
abstract class Term {
  Set<TermPath> get groundPaths;
}
...
class VarTerm extends Term {
  ...
  VarTerm(this.name, {this.isReader = false});
  
  @override
  Set<TermPath> get groundPaths => {};  // Variables have no ground paths
}

/// Structure term: f(t1, ..., tn)
class StructTerm extends Term {
  ...
  Set<TermPath> get groundPaths {
    ...
    for (int i = 0; i < args.length; i++) {
      final argPaths = args[i].groundPaths;
      for (final p in argPaths) {
        paths.add(TermPath([PathElement.functor(functor, arity, i + 1), ...p.elements]));
      }
    }
    return paths;
  }
...
class ListTerm extends Term {
  ...
  Set<TermPath> get groundPaths {
    ...
    final paths = <TermPath>{};
    if (head != null) {
      for (final p in head!.groundPaths) {
        paths.add(TermPath([PathElement.listHead(), ...p.elements]));
      }
    }
    if (tail != null) {
      for (final p in tail!.groundPaths) {
...
class ConstTerm extends Term {
  ...
  @override
  Set<TermPath> get groundPaths => {TermPath([PathElement.constant(value)])};
}
...
/// Procedure call goal
class CallGoal extends Goal {
  final String functor;
  final List<Term> args;
  
  CallGoal(this.functor, this.args);
}
...
TypeCheckResult checkSource(String source, List<Clause> clauses) {
