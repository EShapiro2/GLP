...
### Remark 4.4: Structural Mode vs. Variable Mode
...
Constructs the complement of a moded term per Definition 4.3.
...
Complement (all modes flipped, all variables flipped):
```
↑merge(↑[↑X|Xs], Ys, ↓[↓X?|Zs?])
```
...
- Added Remark 4.4 (Structural Mode vs. Variable Mode)
- Added `variableMatchesStructuralMode()` function
- Added `leafStructuralMode` to ModedPath
- Added `isReal`, `isNumeric` to ModedConstant
- Clarified that `mode` on ModedVariable is implicit mode, not structural mode
- Added interactive type example (HollowIntegers)
- Added mode mismatch negative example
