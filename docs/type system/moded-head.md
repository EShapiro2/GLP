...
**Date**: 2025-01-12  
**Status**: DRAFT  
**Paper References**: Definition 4.8 (Moded Head), Remark 4.X (Mode Correspondence Property)
...
- Nested structure inherits or flips mode based on type complementation:
...
This follows from the construction: argument modes come directly from the type declaration, and nested modes propagate according to type complementation rules.
...
- Nested modes follow type complementation rules
...
computeArgMode(argType, parentMode):
  // Type? flips mode, Type preserves mode
  if argType.isInput:
...
- Head element type is `CounterCall?` (complement because inside Stream?)
...
- Updated algorithms with `getNestedModes()` for type-aware mode computation
- Added interactive type examples (monitor, consumer)
