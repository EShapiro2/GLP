...
```glp
_select(factorial(N, F)) :-
    true |
    factorial(N, F).

_select(fibonacci(N, F)) :-
    true |
    fibonacci(N, F).
...
Each clause matches on the goal term's functor and arity, then calls the corresponding exported procedure with its original arguments.  The fallback clause silently accepts unrecognized goals.
...
This is a system predicate — it uses the `'_activate'` body kernel and is not written by the programmer.  The loop reads goal terms from the input stream, calls `activate` for each, and recurses on the tail.  The core pattern is: read, dispatch, recurse.
...
*Revised from `_select/2` to `_select/1`: removed FCP's Controls/termination circuit argument, which GLP does not use.*
