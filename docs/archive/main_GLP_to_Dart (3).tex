...
\mypara{Single-unit-clause procedures}
To support abstract data types and cleaner code organization, GLP allows procedures defined by a single unit clause (a clause with no guards and no body) to be called in guard position. These are regular procedures with procedure declarations---they have no special status. When such a procedure is called in guard position, the partial evaluator unfolds the call at compile time, applying the bindings from GLP unification to the rest of the clause. When called in body position, it executes as a normal procedure at runtime. If a procedure called in guard position is not defined by a single unit clause, the partial evaluator reports an error. This mechanism is demonstrated in the channel abstractions below.
...
The \verb|send| predicate adds a message to the output stream, \verb|receive| removes a message from the input stream, and \verb|new_channel| creates a pair of channels where each channel's input is paired with the other's output. These procedures can be called in either guard position (unfolded at compile time by the partial evaluator) or body position (executed at runtime). When used as guards in clause heads, they enable readable code that abstracts the underlying stream mechanics:
