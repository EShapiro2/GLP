...
- `integer(X?)` - Test if X is bound to an integer
- `real(X?)` - Test if X is bound to a float
- `atom(X?)` - Test if X is an atom
- `string(X?)` - Test if X is a string
- `constant(X?)` - Test if X is a constant (atom, number, or string)
- `compound(X?)` - Test if X is a compound term
- `var(X?)` - Test if X is unbound
- `nonvar(X?)` - Test if X is bound
...
**Status**: **PLANNED** - Not yet implemented. Requires compiler support.
...
**Implementation**: Will be runtime-implemented (not defined in GLP).
...
**Compiled**:
```
guard_mul(Y?, 2, Temp1?)      % Guard kernel creates Temp1
guard_add(Temp1?, 10, Temp2?) % Guard kernel creates Temp2
X? < Temp2?                   % Guard predicate compares
```
