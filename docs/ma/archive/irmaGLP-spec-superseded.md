...
### 10.2 Callback Pattern (value Request)
...
Bob introduces Alice to Charlie by exporting `[X, X?]` to both.

**Bob creates channel and exports to Alice**:

- `[X, X?]` globalized for Alice:
  - X: spawns `global_send(X?, _w(bob,0), alice)`, no entry
  - X?: entry `(X, alice)` at index 1

**Bob exports to Charlie**:

- `[X, X?]` globalized for Charlie:
  - X: spawns `global_send(X?, _w(bob,2), charlie)`, no entry  
  - X?: entry `(X, charlie)` at index 3

Note: Bob now has two `global_send` goals watching X?, and entries at indices 1 and 3 both pointing to X.

**Alice sends a message**:

Alice assigns her writer (from localizing `_w(bob,0)`), which eventually causes Bob's X to be assigned. Then:

1. X? becomes known at Bob
2. Both `global_send(X?, _w(bob,0), alice)` and `global_send(X?, _w(bob,2), charlie)` fire
3. Messages sent to both Alice and Charlie
4. Both receive and assign their local writers

The value propagates from Alice through Bob to Charlie.
