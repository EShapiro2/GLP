...
**Entry Lifecycle**: The precise rules for when entries are added and removed for each role (created writer, imported writer, created reader, imported reader) are to be further clarified if needed.
...
The Network transaction handles cold-call messages between agents who do not yet share variables. When agent p writes `msg(q, X)` to its network output stream (where q is the destination agent ID and X is the message content):

1. The export helper processes X, updating p's variable table for any variables being sent
2. The transformed message X' is delivered to q's network input stream
3. Any variables in X' that are new to q are added to q's variable table with their creator information

This transaction enables the cold-call protocol: an agent can send an introduction request to another agent via the network, even before any shared variables exist between them. The message content X typically contains a response variable that establishes the initial shared variable for subsequent Communicate transactions.

**Network Streams:**

Each agent p has network input and output streams as part of its initial configuration:
- R_p = ([agent(p, ch(_?, _), ch(_?, _))], ∅, ∅)

The first channel `ch(_?, _)` is the user channel; the second `ch(_?, _)` is the network channel. The network output stream is where p writes `msg(destination, content)` terms; the network input stream is where p receives messages from other agents.
...
| 3.1 | 2026-01-26 | Claude | Added Section 5.4 Network Transaction (Binary) from paper Definition [Implementation-Ready Network Transaction]; renumbered Scheduling to 5.5 |
