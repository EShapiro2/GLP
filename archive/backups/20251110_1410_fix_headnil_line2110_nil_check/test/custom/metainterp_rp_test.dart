...
void main() {
  test('Metainterpreter: run(p(X?)), run(p(X)) - reader then writer', () {
    print('\n' + '=' * 70);
    ...
    print('Expected: First run(p(X?)) suspends, second run(p(X)) binds X=a, first reactivates');
    print('=' * 70 + '\n');
    ...
    // Build p(X?) structure (reader first!)
    const wPXr = 10;
    const rPXr = 11;
    ...
    // Build p(X) structure (writer second!)
    const wPX = 12;
    const rPX = 13;
    ...
    const goalId = 100;
    final env = CallEnv(readers: {0: rPXr, 1: rPX});
    ...
    print('✓ run(p(X?)) suspended on unbound X');
    print('✓ run(p(X)) bound X to a');
    print('✓ run(p(X?)) reactivated and verified p(a) successfully');
  });
}
